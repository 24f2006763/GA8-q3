...
export default async function handler(req, res) {
  ...
  try {
    ...
    // 3. Track duplicates and build failedGates container
    const failedGates = {};
    ...
    for (const vObj of versions) {
      ...
      if (!hasVersionFormatError && policyValid && vObj && typeof vObj === 'object') {
        validVersionObjects.push(vObj);
      }
    }

    // 4. Verify candidate evaluations against gates
